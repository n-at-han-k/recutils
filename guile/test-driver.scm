...
(define script-version "2020-12-07.22") ;UTC
        ...
        (let ((runner
               (lambda ()
                 (test-with-runner
                   ...
                   (load (option 'test-name #f))))))
          (let-values (((coverage-data asd)
                        (with-code-coverage runner)))
            (let ((out (open-output-file "lcov.info")))
